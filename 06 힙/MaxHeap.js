class MaxHeap {
  ...
  heapifyDown(index) {
    while (index < this.heap.length) {
      ...
      let smallest = index;
      if (left < this.heap.length && this.heap[left] > this.heap[smallest]) // left 인덱스가 heap 범위 내이고, smallest보다 크다면
        smallest = left;
      if (right < this.heap.length && this.heap[right] > this.heap[smallest]) // right 인덱스가 heap 범위 내이고, smallest보다 크다면
        smallest = right;
